...
export async function chooseExportPath(defaultName: string): Promise<string | null> {
  ...
  return save({
    ...
    filters: [{ name: "Process Stasis evidence", extensions: ["json"] }],
  });
}
...
function startMockTracking(
  ...
  onMessage({ type: "event", payload: event("attached", `Attached to ${root.comm} (${root.key.pid})`) });
  const timer = window.setInterval(() => {
    ...
    if (sequence === 8) onMessage({ type: "event", payload: event("spawn", "worker-net spawned PID 19748", items[3]) });
    if (sequence === 35) onMessage({ type: "event", payload: event("exit", "worker-io (PID 19731) exited", items[2]) });
    ...
    onMessage({ type: "snapshot", payload: snapshot });
  }, 500);
...
function mockDetails(item: ProcessListItem): ProcessDetails {
  return {
    ...
    namespaces: ["cgroup", "ipc", "mnt", "net", "pid", "time", "user", "uts"].map((name, index) => ({ name, identifier: `${name}:[40265318${index}]` })),
    fileDescriptors: [
