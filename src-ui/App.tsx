...
import {
  ...
  listSessionRecordings, readCaseMetadata, readSessionRecording, setContainmentFrozen,
...
export default function App() {
  ...
  return <ProcessPicker onSelect={setTarget} sessions={sessions} openingSession={opening} onOpenSession={openSession} />;
}
...
function Workspace({ target, historical, onDetach }: { target: ProcessListItem; historical?: HistoricalSession; onDetach: () => void }) {
  ...
  const initialSnapshot = historical?.capture.snapshots.at(-1);
  ...
  const applyContainment = async (freeze: boolean, reason: string, acknowledged: boolean): Promise<ContainmentOutcome | undefined> => {
    setContainmentBusy(true);
    try { const result = await setContainmentFrozen(sessionId, freeze, reason, acknowledged); setContainment(result.status); setToast(`${result.action.action} ${result.action.outcome}: ${result.action.verification}`); return result; }
  ...
  const exportEvidence = async (format: "json" | "html" = "json", includeSensitive = false) => {
    ...
    try {
      ...
      const path = await chooseExportPath(`process-stasis-${target.key.pid}-${timestamp}.${extension}`, extension);
      if (!path) return;
      let capture = recordedCapture;
      if (!capture && recordedAt) capture = await readSessionRecording(sessionId);
      const payload = buildExport({ target, sessionId, snapshot, snapshots, events, details, capture, metadata: caseMetadata, profile, includeSensitive });
  ...
  return <div className={`workspace ${historical ? "historical-workspace" : ""}`} ref={shell}>
    <header className="app-header workspace-reveal">
      ...
      <nav className="workspace-nav" aria-label="Workspace views">{views.map(({ id, label, icon: Icon }) => <button key={id} className={view === id ? "active" : ""} onClick={() => setView(id)} disabled={historical && id === "control"}><Icon weight={view === id ? "fill" : "regular"} /><span>{label}</span></button>)}</nav>
    ...
    {snapshot && !snapshot.rootAlive && <div className="root-exit-banner workspace-reveal"><span className="exit-dot" /><span><strong>Focus process exited.</strong> Its evidence remains available; {historical ? "this recorded session is read-only" : `collection continues for ${survivors.length} known living descendants`}.</span>{!historical && survivors.length > 0 && <button onClick={followSurvivor}>Inspect a survivor</button>}</div>}
    <main className={`workspace-main view-${view} workspace-reveal`} ref={viewHost}>
      ...
      {view === "control" && !historical && <ContainmentPanel status={containment} recording={recording} busy={containmentBusy} onRefresh={refreshContainment} onApply={applyContainment} />}
    </main>
...
function buildExport({ target, sessionId, snapshot, snapshots, events, details, capture, metadata, profile, includeSensitive }: { target: ProcessListItem; sessionId: string; snapshot?: GraphSnapshot; snapshots: GraphSnapshot[]; events: LifecycleEvent[]; details?: ProcessDetails; capture?: RecordedCapture; metadata: CaseMetadata; profile?: CollectorProfile; includeSensitive: boolean }) {
...
function renderHtmlReport(payload: ReturnType<typeof buildExport>) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escape(payload.case.title || "Process Stasis report")}</title><style>body{margin:0;background:#f2f0eb;color:#24252a;font:14px/1.55 system-ui,sans-serif}main{max-width:1100px;margin:auto;padding:56px 32px}h1{font-size:42px;letter-spacing:-.04em;margin:.1em 0}p{color:#656773}.card{background:white;border:1px solid #d9d7d1;border-radius:14px;padding:22px;margin:16px 0}code{font:12px ui-monospace,monospace;color:#3157bf}table{width:100%;border-collapse:collapse}th,td{padding:10px;text-align:left;border-bottom:1px solid #eceae5;font-size:12px}th{color:#767871;text-transform:uppercase;font-size:10px}.meta{display:flex;gap:24px}.meta div{display:grid}.meta span{color:#777;font-size:11px}.meta strong{font-size:18px}</style></head><body><main><small>PROCESS STASIS · SESSION 0.7</small><h1>${escape(payload.case.title || payload.target.command)}</h1><p>${escape(payload.case.summary || "No investigator summary was provided.")}</p><section class="card meta"><div><span>PID</span><strong>${escape(payload.target.pid)}</strong></div><div><span>Snapshots</span><strong>${payload.snapshots.length}</strong></div><div><span>Events</span><strong>${payload.lifecycleEvents.length}</strong></div><div><span>Exported</span><strong>${escape(formatTime(payload.exportedAt))}</strong></div></section><section class="card"><h2>Timeline</h2><table><thead><tr><th>Time</th><th>Event</th><th>Process</th><th>Observation</th><th>Source</th></tr></thead><tbody>${eventRows}</tbody></table></section><section class="card"><h2>Collection limits</h2><ul>${payload.limitations.map((item) => `<li>${escape(item)}</li>`).join("")}</ul></section></main></body></html>`;
}
